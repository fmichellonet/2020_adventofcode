...
pid:745302991 byr:1969 hcl:#7394c7";
        ...
        [Test]
        public void ShouldParseOnePassport()
        {
            const string sample = @"ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
byr:1937 iyr:2017 cid:147 hgt:183cm";

            var d4 = new PassportProcessing(new RequiredPassportValidator());
            var passport = d4.Map(sample);

            passport.Should().HaveCount(1);
            var f = passport.First();
            passport.First().KeysAndValues["iyr"].Should().Be("2017");
        }

        [Test]
        public void MissingMandatoryFieldInvalidatePassport()
        {
...
hcl:#cfa07d byr:1929";
            ...
            var validationResult = validator.Validate(passport.First());

            validationResult.IsValid.Should().Be(false);
        }

        [Test]
        public void MissingCidIsOk()
        {
            const string sample = @"hcl:#ae17e1 iyr:2013
eyr:2024
